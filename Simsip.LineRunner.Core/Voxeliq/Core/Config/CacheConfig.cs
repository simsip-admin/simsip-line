...
namespace Engine.Core.Config
{
    ...
    public class CacheConfig
    {
        ...
        internal bool Validate()
        {
            ...
            if (ViewRange == 0)
                throw new CacheConfigException("ViewDirection range can not be set to zero!");

            if (CacheRange == 0)
                throw new CacheConfigException("Cache range can not be set to zero!");

            if (ViewRange > CacheRange)
                throw new CacheConfigException("ViewDirection range can not be larger than cache range!");
