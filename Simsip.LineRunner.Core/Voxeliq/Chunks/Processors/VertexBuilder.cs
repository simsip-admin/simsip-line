...
namespace Engine.Chunks.Processors
{
    ...
    public class VertexBuilder : GameComponent, IVertexBuilder
    {
        ...
        private void BuildVertexList(Chunk chunk)
        {
            ...
            // chunk.VertexBuffer = new VertexBuffer(this.Game.GraphicsDevice, typeof(VertexPositionColorTexture), vertices.Length, BufferUsage.WriteOnly);
            chunk.VertexBuffer = new VertexBuffer(this.Game.GraphicsDevice, typeof(VertexPositionNormalTexture), vertices.Length, BufferUsage.WriteOnly);
#if DESKTOP
            chunk.VertexBuffer.SimsipSetPrivateData("ChunkVertexBuffer");
#endif
            chunk.VertexBuffer.SetData(vertices);
            chunk.VertexArray = vertices;
...
#if DESKTOP
            chunk.IndexBuffer.SimsipSetPrivateData("ChunkIndexBuffer");
#endif
            chunk.IndexBuffer.SetData(indices);
            chunk.IndexArray = indices;
                                                ...
                                                Color localLightTL, Color localLightTR, Color localLightBL, Color localLightBR)
        {
            ...
            Vector2[] textureUVMappings = TextureHelper.BlockTextureMappings[(int) texture*6 + faceIndex];
        ...
        private static void AddVertex(Chunk chunk, Vector3Int position, Vector3 addition, Vector3 normal, Vector2 textureCoordinate, float sunlight, Color localLight)
        {
            ...
            chunk.VertexList.Add(new VertexPositionNormalTexture(
                position.AsVector3() + addition, 
                normal, 
                textureCoordinate));
        }
