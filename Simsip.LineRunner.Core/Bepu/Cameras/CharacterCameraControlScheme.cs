...
namespace BEPUphysicsDemos
{
    public class CharacterCameraControlScheme : CameraControlScheme
    {
        ...
        public CharacterCameraControlScheme(CharacterController character, Camera camera, Game game /*DemosGame game*/)
            : base(camera, game)
        {
            Character = character;
        ...
        public override void Update(float dt)
        {
            ...
            if (UseCameraSmoothing)
            {
                ...
                Camera.Position = Camera.Position + ConversionHelper.MathConverter.Convert(Character.Body.LinearVelocity * spaceDt);
                ...
                Vector3 error = goalPosition - ConversionHelper.MathConverter.Convert(Camera.Position);
                ...
                if (verticalError > Character.StepManager.MaximumStepHeight)
                {
                    ...
                    Camera.Position -= ConversionHelper.MathConverter.Convert(up * (Character.StepManager.MaximumStepHeight - verticalError));
                ...
                if (horizontalErrorLength > margin * margin)
                {
                    ...
                    Camera.Position -= ConversionHelper.MathConverter.Convert(horizontalError - previousHorizontalError);
                }
                ...
                Camera.Position += ConversionHelper.MathConverter.Convert(up * (verticalError * errorCorrectionFactor));
                // TODO
                // Camera.Position += horizontalError * errorCorrectionFactor;
                Camera.Position += ConversionHelper.MathConverter.Convert(horizontalError * errorCorrectionFactor);
            ...
            else
            {
                ...
                Camera.Position = ConversionHelper.MathConverter.Convert(Character.Body.Position + (Character.StanceManager.CurrentStance == Stance.Standing ? StandingCameraOffset : CrouchingCameraOffset) * Character.Body.OrientationMatrix.Up);
            }
        }
    }
}
