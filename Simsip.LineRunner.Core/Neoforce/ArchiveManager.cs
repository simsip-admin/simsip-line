...
#if NETFX_CORE
    public static async Task<ArchiveManager> FactoryArchiveManager(IServiceProvider serviceProvider, string archive=null)
    {
        var archiveManager = new ArchiveManager(serviceProvider, null);
        await archiveManager.FactoryArchiveManagerInitialize(archiveManager, archive);

        return archiveManager;
    }

    private async Task FactoryArchiveManagerInitialize(ArchiveManager archiveManager, string archive)
    {
        this.archive = await ZipFile.Read(archive);
    }
#endif
...
#if NETFX_CORE
      else if (await FileUtils.FolderExistsAsync(path))
      {
          IList<string> folderNames = await FileUtils.GetFolderNamesAsync(path);
        string[] dirs = ((List<string>)folderNames).ToArray();
#else
