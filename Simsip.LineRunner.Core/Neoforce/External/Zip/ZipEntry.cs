      ...
      private bool _TrimVolumeFromFullyQualifiedPaths= true;  // by default, trim them.
        ...
        private static bool ReadHeader(System.IO.Stream s, ZipEntry ze)
        {
            ...
            if (SignatureIsNotValid(signature))
            {
                ...
                {
#if NETFX_CORE
                    Debug.WriteLine("  ZipEntry::Read(): Bad signature ({0:X8}) at position {1}", signature, s.Position);
#else
...
#if NETFX_CORE
        internal async static Task<ZipEntry> Create(String filename)
#else
...
#endif
        {
            ...
            StorageFile zipFile = await zipFolder.GetFileAsync(filename);
            BasicProperties basicProperties = await zipFile.GetBasicPropertiesAsync();
            entry._LastModified = basicProperties.DateModified.DateTime;
#else
...
#if NETFX_CORE
        private async void Extract(string basedir, System.IO.Stream s)
#else
...
#endif
        {
            ...
            if (basedir != null)
            {
                ...
                {
#if NETFX_CORE
                    if (!await FileUtils.FolderExistsAsync(TargetFile))
                    {
                        await FileUtils.CreateFolderAsync(TargetFile);
                    }
#else
            ...
            using (System.IO.MemoryStream memstream = new System.IO.MemoryStream(_FileData))
            {
                ...
                try
                {
...
#if NETFX_CORE
                        if (!await FileUtils.FolderExistsAsync(System.IO.Path.GetDirectoryName(TargetFile)))
                        {
                            await FileUtils.CreateFolderAsync(System.IO.Path.GetDirectoryName(TargetFile));
                        }
#else
                    ...
                    try
                    {
                        ...
                        {
                            StorageFolder zipFolder = ApplicationData.Current.LocalFolder;
                            StorageFile zipFile = await zipFolder.CreateFileAsync(TargetFile, CreationCollisionOption.ReplaceExisting);
                            IRandomAccessStream randomAccessStream = await zipFile.OpenAsync(FileAccessMode.Read);

                            IOutputStream outputStream = randomAccessStream.GetOutputStreamAt(0);
                            output = outputStream.AsStreamForWrite();
                        }
#else
                        if (TargetFile != null)
                        {
                        ...
                        {
#if NETFX_CORE
                            Debug.WriteLine("{0}: _FileData.Length= {1}", TargetFile, _FileData.Length);
                            Debug.WriteLine("{0}: memstream.Position: {1}", TargetFile, memstream.Position);
#else
                            ...
                            if (n > 1000)
                            {
                                n = 500;
#if NETFX_CORE
                                Debug.WriteLine("{0}: truncating dump from {1} to {2} bytes...", TargetFile, _FileData.Length, n);
#else
                            ...
                            for (int j = 0; j < n; j += 2)
                            {
...
#if NETFX_CORE
                                Debug.WriteLine(" {0:X2}", _FileData[j]);
#else
                                ...
                                {
#if NETFX_CORE
                                    Debug.WriteLine("{0:X2}", _FileData[j + 1]);
#else
                        ...
                        while (n != 0)
                        {
                            ...
                            {
#if NETFX_CORE
                                Debug.WriteLine("{0}: about to read...", TargetFile);
#else
                            ...
                            {
#if NETFX_CORE
                                Debug.WriteLine("{0}: got {1} bytes", TargetFile, n);
#else
                            ...
                            if (n > 0)
                            {
                                ...
                                {
#if NETFX_CORE
                                    Debug.WriteLine("{0}: about to write...", TargetFile);
#else
        ...
        internal void WriteCentralDirectoryEntry(System.IO.Stream s)
        {
            ...
            for (j = 0; j < Header.Length - 30; j++)
            {
                ...
                {
#if NETFX_CORE
                    Debug.WriteLine(" {0:X2}", bytes[i + j]);
#else
...
#if NETFX_CORE
        private async void WriteHeader(System.IO.Stream s, byte[] bytes)
#else
...
#endif
        {
            ...
            Stream input = inputStream.AsStreamForRead();
            crc = crc32.GetCrc32AndCopy(input, CompressedStream);
            input.Dispose();
            CompressedStream.Dispose();  // to get the footer bytes written to the underlying stream
#else
            ...
            {
#if NETFX_CORE
                Debug.WriteLine("Uncompressed Size: {0}", crc32.TotalBytesRead);
#else
	    ...
	    // behavior.  It only affects zip creation.
	      ...
	      FileName.ToCharArray();
            ...
            {
#if NETFX_CORE
                Debug.WriteLine("local header: writing filename, {0} chars", c.Length);
                Debug.WriteLine("starting offset={0}", i);
#else
            ...
            {
#if NETFX_CORE
                Debug.WriteLine("preserving header of {0} bytes", _header.Length);
#else
        ...
        internal void Write(System.IO.Stream s)
        {
            ...
            {
#if NETFX_CORE
                Debug.WriteLine("{0}: writing compressed data to zipfile...", FileName);
                Debug.WriteLine("{0}: total data length: {1}", FileName, _UnderlyingMemoryStream.Length);
#else
            ...
            while ((n = _UnderlyingMemoryStream.Read(bytes, 0, bytes.Length)) != 0)
            {
                ...
                {
#if NETFX_CORE
                    Debug.WriteLine("{0}: transferring {1} bytes...", FileName, n);
#else
                    ...
                    for (int j = 0; j < n; j += 2)
                    {
                        ...
                        {
#if NETFX_CORE
                            Debug.WriteLine("{0:X2}", bytes[j + 1]);
#else
