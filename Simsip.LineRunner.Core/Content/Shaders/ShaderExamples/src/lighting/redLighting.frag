// This fragment shader computes a lighting effect. It's based
// on the Blinn-Phong lighting model.
...
void main(void)
{
	...
	light1.ambient = vec4(0.1, 0.1, 0.0, 1.0);
	light1.diffuse = vec4(0.9, 0.1, 0.1, 1.0);
	light1.specular = vec4(0.9, 0.6, 0.6, 1.0);

	/* Set up the second light */
	light2.halfVector = vec3(-0.3, -0.2, -0.1);
	light2.position = vec3(-2.0, -2.0, -8.0);
	light2.ambient = vec4(0.1, 0.0, 0.0, 1.0);
	light2.diffuse = vec4(0.7, 0.3, 0.1, 1.0);
	light2.specular = vec4(0.9, 0.3, 0.3, 1.0);
	...
	/* Assign the lighting result to the fragment color */	
