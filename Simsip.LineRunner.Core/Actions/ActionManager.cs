...
namespace Simsip.LineRunner.Actions
{
    ...
    {
        private static GameModel[] m_pTmpKeysArray = new GameModel[128];
        ...
        public void Update(float dt)
        {
            ...
            {
                m_pTmpKeysArray = new GameModel[m_pTmpKeysArray.Length * 2];
            }
        ...
        public void AddAction(Action action, GameModel target, bool paused)
        {
        ...
        public void RemoveAllActionsFromTarget(GameModel target)
        {
            if (target == null)
            {
                return;
        ...
        public void RemoveAction(Action action)
        {
            ...
            if (m_pTargets.TryGetValue(target, out element))
            {
                ...
                {
                    Debug.WriteLine("cocos2d: removeAction: Action not found");
                }
            }
            else
            {
        ...
        public void RemoveActionByTag(int tag, GameModel target)
        {
            ...
            if (m_pTargets.TryGetValue(target, out element))
            {
                ...
                Debug.WriteLine("cocos2d : removeActionByTag: Tag " + tag + " not found");
            }
            else
            {
                Debug.WriteLine("cocos2d : removeActionByTag: Target not found");
            }
        }

        public Action GetActionByTag(int tag, GameModel target)
        {
            Debug.Assert(tag != (int) ActionTag.Invalid);
            ...
            if (m_pTargets.TryGetValue(target, out element))
            {
                if (element.Actions != null)
                {
                    ...
                    Debug.WriteLine("cocos2d : getActionByTag: Tag " + tag + " not found");
                }
            }
            else
            {
        ...
        public int NumberOfRunningActionsInTarget(GameModel target)
        {
            HashElement element;
            if (m_pTargets.TryGetValue(target, out element))
            {
